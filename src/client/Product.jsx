...
const Product = ({product, name, tooltip, productionRate}) => {
    ...
    const badgeItem = productionRate
        ? (<div className="text-xs absolute left-[15px] whitespace-nowrap bottom-[-8px] pl-1 pr-1 bg-blue-200 outline outline-white outline-2 rounded-full">{productionRate}</div>)
        : (<></>)

    let width = ""
    if (productionRate) {
        switch (productionRate.length) {
            case 1:
                width = "w-[2em]"
                break
            case 2:
